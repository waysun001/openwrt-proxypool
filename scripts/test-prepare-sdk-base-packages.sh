...
mkdir -p \
	"$SDK_ROOT/package/kernel/linux" \
	"$SDK_ROOT/package/toolchain" \
	"$OPENWRT_ROOT/package/libs/libmnl" \
	"$OPENWRT_ROOT/package/network/config/firewall4" \
	"$OPENWRT_ROOT/package/kernel/cryptodev-linux" \
	"$OPENWRT_ROOT/package/kernel/linux" \
	"$OPENWRT_ROOT/package/toolchain"

printf '%s\n' 'sdk kernel marker' >"$SDK_ROOT/package/kernel/linux/marker"
...
printf '%s\n' 'base cryptodev marker' >"$OPENWRT_ROOT/package/kernel/cryptodev-linux/Makefile"
printf '%s\n' 'source kernel marker' >"$OPENWRT_ROOT/package/kernel/linux/marker"
...
grep -Fqx 'base cryptodev marker' "$SDK_ROOT/package/kernel/cryptodev-linux/Makefile"
grep -Fqx 'sdk kernel marker' "$SDK_ROOT/package/kernel/linux/marker"
