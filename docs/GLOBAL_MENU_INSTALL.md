...
**替换 LuCI 原有菜单**为智联盒子的快捷导航和统计信息，简化界面，只保留常用功能。
...
```
┌───────────────────────────────────────────────────────────────────┐
│ [智联盒子] [系统日志] [信道分析] [备份与升级] [无线] [重启]  总 5  已连接 3  未连接 2 │
└───────────────────────────────────────────────────────────────────┘
│ （LuCI 原有菜单已隐藏）                                             │
```
...
⚠️ **重要提示**：此脚本会**隐藏 LuCI 原有菜单**（状态|系统|服务|网络|注销），仅保留自定义快捷菜单。

1. **备份重要**：脚本会自动备份原始 `header.htm`，请勿删除备份文件
2. **菜单隐藏**：安装后无法通过顶部菜单访问"系统"、"服务"等页面，需通过直接 URL 访问
3. **恢复方法**：执行卸载脚本可完整恢复 LuCI 原有菜单和界面
4. **升级影响**：升级 LuCI 后可能覆盖修改，需重新执行安装脚本
5. **兼容性**：适用于标准 LuCI 界面，自定义主题可能不兼容
6. **统计更新**：统计数字仅在智联盒子页面自动刷新（10秒一次）

### 访问隐藏页面

如需访问 LuCI 原有菜单的其他页面，直接输入 URL：

```
http://192.168.1.1/cgi-bin/luci/admin/status/overview  # 状态概览
http://192.168.1.1/cgi-bin/luci/admin/system           # 系统设置
http://192.168.1.1/cgi-bin/luci/admin/services         # 服务管理
http://192.168.1.1/cgi-bin/luci/admin/network          # 网络配置
```
