...
7. input 只允许 DHCP、ProxyPool DNS、TCP 80/443 管理页及经过精确透明重定向授权的 TCP；SSH 和其他路由器 listener 默认拒绝。
8. guardian 先撤权再停节点；防火墙重载先重建空集合再有界 resync。任何失败结果必须是断网。
9. flow offload 关闭；二层隔离同时依赖 AP isolation、bridge-port isolation 和 bridge-family nft 软件兜底。
...
- [ ] `lan` zone 设置 input/forward reject；添加命名 DHCP、DNS、80/443 input rules。
...
- [ ] OpenWrt 23.05.3 `default_postinst` 会在 live opkg 中无条件调用包内 init 的 `start`：guardian 的普通 `start_service` 必须 no-op，真实 S18 行为放在 `boot()`；reload/stop 仍撤权。upgrade custom postinst 显式 enable S18。live uci-default 遇 active flowtable 非零并保留；cold wrapper 在 S19 finalize 前不能被 boot 删除。
...
- [ ] 后续安全 DNS 数据面完成前，本任务只保证客户端收到 SERVFAIL/安全断网，不以 ISP DNS 维持可用性。节点 endpoint 自身若使用域名，也必须明确报告当前无法安全解析，不能偷偷走 WAN resolver。
...
- [ ] 从 UCI/runtime 枚举 br-lan 实际成员，不硬编码 `lan1..lan4`。
- [ ] 每个有线 bridge port 设置 isolate；每个 LAN AP 设置 `isolate=1` 与 `bridge_isolate=1`。network restart、端口重新加入 bridge 和 Wi-Fi reload 后由有界 hotplug reconciliation 重施并验证，失败保持 bridge guardian 拒绝。
- [ ] bridge-family guardian 拒绝 `ibrname br-lan -> obrname br-lan` 的所有 bridge forward 帧。
- [ ] core package 显式依赖 `firewall4` 与 `kmod-nft-bridge`，并把 reconciliation/hotplug 安装到精确路径和 0755 mode；IPK inspector 对缺文件、错路径、错 mode 逐项失败。
- [ ] 禁止 flow offload；host/CI 只宣称软件配置和 reconciliation 合同通过，DSA/hardware offload 是否遵守 isolated port 必须通过真机门禁后才能宣称永久硬件 L2 隔离。
...
- [ ] sysupgrade keep list 精确保留 `proxypool_v2`、`proxypool_runtime`、`/etc/proxypool/activated-backend` 与 `/etc/proxypool/cleanup-required`，同时 package conffiles 仍只有 V1 `/etc/config/proxypool`；不得把持久 backend 证据塞进 package conffiles 或用镜像默认值覆盖。
...
- [ ] 运行全部 host tests、Go tests/vet、shell syntax、`git diff --check`、Linux nft syntax、SDK core/LuCI IPK inspector 与 ImageBuilder OpenWrt 23.05.3 `fw4 check/print`。宿主机任意 nft 版本的 `nft -c` 只作补充，不能替代目标版本门禁。
- [ ] 独立安全评审检查所有安全不变量、cleanup 所有权和升级/回滚路径。
- [ ] 即使 host/CI 全绿，本 gate 的真机轮次也只确认 guardian、管理白名单、V1 安全回滚边界和 firewall reload；这些结果作为后续解锁的必要证据，但本身不解除跨 backend 阻断。
...
- OpenWrt 23.05.3 SDK package build and actual IPK inspection.
- ImageBuilder `fw4 check`, `fw4 print`, rootfs file modes and init symlinks.
- GL-MT6000 four-round hardware matrix: cold boot/stop/crash/reload leakage、管理白名单、预置节点的 L2TP/SOCKS5/SLP 安全路径、LAN/Wi-Fi 隔离和 sysupgrade 保留合同。普通域名浏览、40–60 节点批量稳定、成功 restore 与 V2 正式数据面由后续阶段门禁。
